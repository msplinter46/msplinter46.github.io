...
regex="LEGPH"        #elimina resultados da estirpe em estudo
regex2="LEGP.*"      #remove resultaods do blast da espécie Legionella pneumophila       
e_thresh=0.05        #e-value maximo que irá ser aceite

    
dump = open("dump_nr.txt","w")     #ficheiro onde serão guardados resultados
dump2=open("dump2_nr.txt","w")
            ...
            for hsp in alignment.hsps:                     
                if hsp.expect<e_thresh:      
                    if not search(regex, alignment.hit_id):
                       ...
                       if not search(regex, alignment.hit_id) and search(regex2, alignment.hit_id) :    #procura por resultados de legionella que não seja Philadelphia
                        hit_id=alignment.hit_id.strip("gi")
                        if search("sp", hit_id):
                            hit_id1=str(search("(.*)sp(.*)", hit_id).group(1)).strip("|\s")
                            hit_id2=str(search("(.*)sp(.*)", hit_id).group(2)) 
                            ...
                            if search(regex_name, name):
                                ...
                                if search(regex_name, name):
                                    name=search(regex_name, name).group(1)                         
                            dump2.write(hit_id1+hit_id2+"\t"+name +"\t" 
                                       +str(hsp.expect)+"\t"+str(hsp.score)+"\t"+str(hsp.align_length)+"\t"+function+"\n")
