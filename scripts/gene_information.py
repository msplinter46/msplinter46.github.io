...
def gerar_info(records):            #gera uma tabela com informações do gene e proteina e grava a tabela num ficheiro.
    ...
    res=[]
    res.append("GeneID"+"\t"+ "Accession Number"+"\t"+"Locus Tag"+"\t"+"Gene"+"\t"+"Strand"+"\t"+ "Biological Function"+"\t"+
        ...
        for a in r.features:
            if a.type == "CDS" and a.qualifiers["db_xref"][0][7::] in genesid:
                ...
                if "gene" in a.qualifiers:              
                    ...
                    gene_name= "null" 
                gene_desc=a.qualifiers['function'][0]
                strand=str(a.strand)
                protein_id=str(a.qualifiers['protein_id'][0])
                res.append(gene_id+"\t"+ accession+"\t"+ locus +"\t"+ gene_name +"\t"+ strand +"\t"+gene_desc +"\t"+protein_id +"\t"+ "\n") 
